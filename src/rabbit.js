...
function repeat() {
    var repeatAnimation = animation().loadImage(images)
        .changePosition($rabbit1, rightRunningMap, images[0]).repeatForever();
...
function run() {
    ...
    var runAnimation = animation().loadImage(images).enterFrame(function (success, time) {
        var ratio = (time) / interval;//间隔时间
        var position;
        var left;
        if (right) {
            position = rightRunningMap[frame].split(" ");//背景图位置
...
function lose() {
    var loseAnimation = animation().loadImage(images).changePosition($rabbit4, rabbitLoseMap, images[1]).repeat(3).then(function () {
        console.log("lose animation repaet 1 times finished");
